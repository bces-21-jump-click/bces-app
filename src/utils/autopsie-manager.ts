...
import type { Autopsie, Injury } from '@/models/autopsie'
  ...
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      injuries.forEach((injury, index) => {
        const colorKey = INJURY_COLORS[index]
        const rgb = colorKey ? COLOR_MAP[colorKey] : undefined
        const hex = rgb ? `rgb(${rgb.join(',')})` : (colorKey ?? 'black')
        ...
        if (injury.points.length > 0) {
          ...
          for (let i = 0; i < index; i++) {
            const prev = injuries[i]
            if (prev && prev.points.length > 0) {
              const otherAvgY = prev.points.reduce((sum, p) => sum + p.y, 0) / prev.points.length
...
export async function generateReport(
  ...
  const drawTextSection = (title: string, text?: string): void => {
    ...
    const blockHeight = 12 + (lines as string[]).length * lineHeight + 4
  ...
  try {
    ...
    const GState = (pdf as unknown as Record<string, new (opts: Record<string, number>) => object>)
      .GState!
